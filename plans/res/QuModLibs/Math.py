...
class Vec3(object):
    ...
    def rotateVector(self, axis, angle):
        ...
        # 保存原始分量，避免 in-place 赋值污染（原版 vector=self 无拷贝的 bug 修复）
...
class QBox3D:
    ...
    def overlapsSAT(self, otherBox):
        ...
        """ 使用分离轴定理 (SAT) 进行碰撞检测，检测两个旋转后的Box3D是否重叠
            完整 OBB vs OBB：检测 A的3轴 + B的3轴 + A×B的9叉积轴，共15条分离轴候选 """
        import math as _math

        def _getLocalAxes(box):
            """ 获取盒子自身三条局部轴（世界坐标系方向） """
            angle = _math.radians(box.rotationAngle)
            cos_a = _math.cos(angle)
            sin_a = _math.sin(angle)
            ax = box.rotationAxis
            ux, uy, uz = ax.x, ax.y, ax.z
            # Rodrigues 旋转：将标准轴 (1,0,0), (0,1,0), (0,0,1) 旋转到盒子局部方向
            # 与 Vec3.rotateVector 使用相同的 sin 符号约定
            def _rot(ex, ey, ez):
                ox, oy, oz = ex, ey, ez
                rx = (cos_a + (1-cos_a)*ux*ux)*ox + ((1-cos_a)*ux*uy + sin_a*uz)*oy + ((1-cos_a)*ux*uz - sin_a*uy)*oz
                ry = ((1-cos_a)*ux*uy - sin_a*uz)*ox + (cos_a + (1-cos_a)*uy*uy)*oy + ((1-cos_a)*uy*uz + sin_a*ux)*oz
                rz = ((1-cos_a)*ux*uz + sin_a*uy)*ox + ((1-cos_a)*uy*uz - sin_a*ux)*oy + (cos_a + (1-cos_a)*uz*uz)*oz
                return Vec3(rx, ry, rz)
            return [_rot(1,0,0), _rot(0,1,0), _rot(0,0,1)]

        def _project(corners, axis):
            dots = [Vec3.dot(c, axis) for c in corners]
            return min(dots), max(dots)

        def _separated(aMin, aMax, bMin, bMax):
        ...
        aAxes = _getLocalAxes(self)
        bAxes = _getLocalAxes(otherBox)

        # A的3轴 + B的3轴
        for axis in aAxes + bAxes:
            length = _math.sqrt(axis.x**2 + axis.y**2 + axis.z**2)
            if length < 1e-8:
                continue
            aMin, aMax = _project(aCorners, axis)
            bMin, bMax = _project(bCorners, axis)
            if _separated(aMin, aMax, bMin, bMax):
                return False
